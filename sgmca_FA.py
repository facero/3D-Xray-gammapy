...
#import IAE_JAX_v2_devl as IAE_JAX
...
def reconstruct_comp(Sources,g_A):
    ...
    """
    Reconstruct cubes for each components from GMCA outputs
    """
...
def sgmca(X, n, **kwargs):
    ...
    nbItMin1 = kwargs.get('nbItMin1', 200)
    ...
    doSemiBlind = kwargs.get('doSemiBlind', False)
    ...
    if doSemiBlind:
        ...
        for fname in models:
            #print('FNAME, models=', fname,models)
            ...
            IAEModelsInfo.append({'nb_sources': models[fname],'models_name': fname,
    ...
    if AInit is None:
        ...
        print('Loading Ainit skipping GMCA step 1')
        A = AInit.copy()
    ...
    while True:
        ...
        #for i in range(n):
        #    plt.plot(A[:,i], label=f'A[{i}] it={it} top loop')
        #plt.legend() ; plt.show()
        if verb >= 2:
            if it%10 == 0:
        ...
        #for i in range(n):
        #    plt.plot(A[:,i], label=f'A[{i}]  it={it} updated A')
        #plt.legend() ; plt.show()
        if step > 1:
            ...
            # --- Application of the model-based constraint with OMP step
#            print('IAEModels=',IAEModels)
#            for l, IAEModel in enumerate(IAEModels):
#                print('l=',l)
#                print(f"Source  {l} mapped to {IAEModelsInfo[l]['models_name']}")
#                print("Sources not mapped: ", not_mapped_sources)
#                print("IAEModelsInfo[l]['sources']=",IAEModelsInfo[l]['sources'][l])
#                print("A[:, IAEModelsInfo[l]['sources'][l]].T.shape",A[:, IAEModelsInfo[l]['sources'][l]].T.shape)
#                output = IAEModel.barycentric_span_projection(A[:, IAEModelsInfo[l]['sources'][l]].T,
#                                                              Lambda0=IAEModelsInfo[l]['Lambda0'],
#                                                              Amplitude0=IAEModelsInfo[l]['Amplitude0'], niter=nbItProj)
#
#                plt.plot(A[:, IAEModelsInfo[l]['sources'][l]], label='Original data')
#                print('Shape=', A[:, IAEModelsInfo[l]['sources'][l]].shape)
#                plt.plot(output['XRec'].T, label='BSP result')
#                plt.yscale('log') ; plt.title(f"Source {l} mapped to {IAEModelsInfo[l]['models_name']}"); plt.legend();plt.show()
#
#                A[:, IAEModelsInfo[l]['sources']] = cp.copy(cp.copy(output['XRec'].T))


            # --- Application of the model-based constraint with user specified order 
            #print('IAEModels=',IAEModels)
            #print('(A,S).shape=', A.shape, S.shape)

            Aproj_index=[1]
            print(IAEModels)
            IAEModel=IAEModels[0] #same model for comp 1 and 2
#            for l, IAEModel in enumerate(IAEModels):
            for l in Aproj_index:

                print('l=',l)
                idx=l
                #idx=Aproj_index[l]
#                print(f"Spectrum A[{Aproj_index[l]}] projected to model {IAEModelsInfo[l]['models_name']}")
                print(f"Spectrum A[{idx}] projected to model {IAEModelsInfo[0]['models_name'].split('/')[-1]}")

                #print("Sources not mapped: ", not_mapped_sources)
                #print("IAEModelsInfo[l]['sources']=",IAEModelsInfo[l]['sources'][l])
                #print('IAEModel=', IAEModel)
                #print('A[:, Aproj_index[l]].T.shape=', A[:, [Aproj_index[l]] ].T.shape)

                output = IAEModel.barycentric_span_projection(A[:, [idx]].T, niter=nbItProj, step_size=0.1)
                output2 = IAEModel.fast_interpolation(A[:, [idx]].T)
                plt.plot(A[:, idx], label='Original data')
                #print('Shape=', A[:, Aproj_index[l]].shape)
                plt.plot(output['XRec'].T, label='BSP result')
                plt.plot(output2['XRec'].T, label='FI result')

                plt.ylim(3e-3,1)

                plt.yscale('log') ; plt.title(f"Source {IAEModelsInfo[0]['models_name'].split('/')[-1]} mapped to {idx} it={it}"); plt.legend();plt.show()
                A[:, [idx] ] = cp.copy(cp.copy(output['XRec'].T))


            for i in range(n):
                plt.plot(A[:,i], label=f'Aproj[{i}]')
                plt.yscale('log')
                plt.ylim(1e-3,1)
            plt.legend() ; plt.show()
